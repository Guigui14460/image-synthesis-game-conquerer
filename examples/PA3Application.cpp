      ...
      m_renderMode(GL_TRIANGLES)
{
  ...
  initGLState();
  makeASphere(40, 40, glm::vec3(0.f));
  makeATorus(40, 40, 2.f, glm::vec3(1.f, 0.f, 0.f));
  makeAShell(40, 40, glm::vec3(0.f, 1.f, 1.f));
}

std::shared_ptr<VAO> PA3Application::makeParamSurf(DiscreteLinRange rgPhi, DiscreteLinRange rgTheta, const std::function<glm::vec3(float, float)> & posFunc, bool isCyclicInPhi, bool isCyclicInTheta, glm::vec3 color)
{
  ...
  for(uint i = 0; i < rgPhi.nbVals; i++){
    for(uint j = 0; j < rgTheta.nbVals; j++){
      glm::vec3 phiThetaVector = posFunc(rgPhi.value(i), rgTheta.value(j));
      glm::vec3 phi1ThetaVector = posFunc(rgPhi.value(i+1), rgTheta.value(j));
      glm::vec3 phiTheta1Vector = posFunc(rgPhi.value(i), rgTheta.value(j+1));
      glm::vec3 phi1Theta1Vector = posFunc(rgPhi.value(i+1), rgTheta.value(j+1));

      std::vector<glm::vec3>::iterator it;
      it = std::find(positions.begin(), positions.end(), phiThetaVector);
      uint phiTheta = it - positions.begin();
      if (it == positions.end()){
        positions.push_back(phiThetaVector);
        colors.push_back(color);
      }

      it = std::find(positions.begin(), positions.end(), phi1ThetaVector);
      uint phi1Theta = it - positions.begin();
      if (it == positions.end()){
        positions.push_back(phi1ThetaVector);
        colors.push_back(color);
      }

      it = std::find(positions.begin(), positions.end(), phiTheta1Vector);
      uint phiTheta1 = it - positions.begin();
      if (it == positions.end()){
        positions.push_back(phiTheta1Vector);
        colors.push_back(color);
      }

      it = std::find(positions.begin(), positions.end(), phi1Theta1Vector);
      uint phi1Theta1 = it - positions.begin();
      if (it == positions.end()){
        positions.push_back(phi1Theta1Vector);
        colors.push_back(color);
      }
      if((i <= rgPhi.nbVals - 2 or isCyclicInPhi) and (j <= rgTheta.nbVals - 2 or isCyclicInTheta)){
        // first triangle
        ibo.push_back(phiTheta);
        ibo.push_back(phi1Theta);
        ibo.push_back(phiTheta1);

        // second triangle
        ibo.push_back(phiTheta1);
        ibo.push_back(phi1Theta);
        ibo.push_back(phi1Theta1);
      }
    }
  }
...
void PA3Application::makeASphere(unsigned int nbPhi, unsigned int nbTheta, glm::vec3 color)
{
  auto posFunc = [](float phi, float theta) { return glm::vec3(cos(phi) * sin(theta), sin(phi) * sin(theta), 1 - cos(theta)); };

  const float pi = glm::pi<float>();
  std::shared_ptr<VAO> vao = makeParamSurf(DiscreteLinRange(nbPhi, 0, 2 * pi), DiscreteLinRange(nbTheta, 0, pi), posFunc, true, false, color);
  glm::mat4 mw(1);
  mw = glm::scale(mw, {0.2, 0.2, 0.2});
  m_vaos.push_back(RenderObject::createInstance(m_program, vao, mw));
}

void PA3Application::makeATorus(unsigned int nbPhi, unsigned int nbTheta, float smallRadius, glm::vec3 color)
{
  auto posFunc = [&smallRadius](float phi, float theta) { return glm::vec3((smallRadius + cos(theta)) * cos(phi), sin(theta), (smallRadius + cos(theta)) * sin(phi)); };

  const float pi = glm::pi<float>();
  std::shared_ptr<VAO> vao = makeParamSurf(DiscreteLinRange(nbPhi, 0, 2 * pi), DiscreteLinRange(nbTheta, 0, 2 * pi), posFunc, true, false, color);
  glm::mat4 mw(1);
  mw = glm::translate(mw, {0.5, 0, 0});
  mw = glm::rotate(mw, 3 * pi / 4, {1, 0, 1});
  mw = glm::scale(mw, {0.2, 0.2, 0.2});
  m_vaos.push_back(RenderObject::createInstance(m_program, vao, mw));
}

void PA3Application::makeAShell(unsigned int nbPhi, unsigned int nbTheta, glm::vec3 color)
{
  ...
  const float pi = glm::pi<float>();
  std::shared_ptr<VAO> vao = makeParamSurf(DiscreteLinRange(nbPhi, 0, pi), DiscreteLinRange(nbTheta, -pi / 4, 5 * pi / 2), posFunc, false, false, color);
  glm::mat4 mw(1);
  mw = glm::translate(mw, {-0.5, 0, 0});

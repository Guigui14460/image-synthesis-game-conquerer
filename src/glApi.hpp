...
{
  this->m_attributeType = AttributeProperties<T>::typeEnum;
  this->m_attributeCount = values.size();
  this->m_attributeSize = AttributeProperties<T>::components;

  this->bind();
  glBufferData(this->m_target, this->m_attributeCount * this->m_attributeSize * sizeof(this->m_attributeType), values.data(), GL_STATIC_DRAW);
  this->unbind();
}

template <typename T> void VAO::setVBO(uint attributeIndex, const std::vector<T> & values)
{
  if(attributeIndex < 0 or attributeIndex >= m_vbos.size()){
    throw "ERROR::VAO::setIBO : atributeIndex out of bounds";
  }
